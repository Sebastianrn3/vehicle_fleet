...
class Vehicle:
    ...
    def if_driver_can_work(self, driver, when: date = date.today()):
        # check for license
        if not self.driver_category_required in driver.categories_present:
        ...
        if driver.vacation_date != None and when != None and driver.vacation_date[0] <= when <= driver.vacation_date[1]:
    ...
    def if_papers_in_order(self):
        inspection =  self.check_inspection_term()
        insurance = self.check_insurance_term()
        if inspection and insurance:
            print("Dokumentai tvarkingi.")
            return True
        print("Sutvarkykite dokumentus!")
        return False

    def check_inspection_term(self):
    ...
    def calculate_cost_per_km(self):
        # ! Expenses for vehicle only, excluding driver's fee
        # apply .try for improving
        cost_per_km = self.fixed_costs / self.annual_mileage + self.fuel_consumption/100 * PRICES[self.fuel_type]
        print(f"{self.fuel_consumption}l/100km,{self.fuel_type}({PRICES[self.fuel_type]}e/l), +fix: {cost_per_km}e/km")

        self.if_papers_in_order()
...
class Bus(Vehicle):
    ...
    def calculate_estimate_cost(self, passengers: int, distance: int, driver):
        # checking drivers capability
        if not self.if_driver_can_work(driver, when = None):
            return False

        number_of_trips = self.calculate_trips(passengers = passengers)
        total_cost = distance * number_of_trips * (self.calculate_cost_per_km() + driver.charge_for_km) // 1
        print(f"Nuvežimo savikaina {total_cost}e")
        return total_cost
...
class Truck(Vehicle):
    ...
    def if_driver_can_work(self, driver, when: date = date.today()):
        ...
        if not self.driver_category_required in driver.categories_present:
            print(f"{driver.fullname} neturi tinkamos kategorijos.")
            return False
        print(f"{driver.fullname} gali vairuoti sunkvežinmius")
        if "E" in driver.categories_present:
            print(f"...su priekaba")
...
# vacation_period = (datetime.date(2025, 7, 15), datetime.date(2025, 8, 14))
# print(type(vacation_period))
# print(vacation_period)